...
const Studregister = () => {
    ...
    const Register = async () => {
        ...
        try {
            const response = await fetch('http://localhost:3001/student/add', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ name, email, password, department: selectedDepartment, type: 'student', rollnumber, program }),
            });
            const data = await response.json();
        ...
        <div className="container d-flex justify-content-center align-items-center vh-100">
            <div className="card p-4">
                <form>
                    ...
                    <div className="mb-3 row">
                        <label htmlFor="inputProgram" className="col-sm-4 col-form-label">Program</label>
                        <div className="col-sm-8">
                            <input type="text" value={program} onChange={(ev) => setProgram(ev.target.value)} className="form-control" id="inputProgram" required/>
                        </div>
                    </div>
                    <div className="mb-3 row">
                        <label htmlFor="inputRollnumber" className="col-sm-4 col-form-label">Roll Number</label>
                        <div className="col-sm-8">
                            <input type="text" value={rollnumber} onChange={(ev) => setRollnumber(ev.target.value)} className="form-control" id="inputRollnumber" required/>
                        </div>
                    </div>
                    <div className="mb-3 row">
